...
$(function() {
    ...
    function menuItem() {
        ...
        if (flag) {
			...
			//$('.mainContent').parents('.mainContent').append(str1);
			...
			//var frameObj = $('.mainContent').find('iframe.RuoYi_iframe').eq(0);
			var iframe = $('.mainContent').find('iframe.RuoYi_iframe').eq(0).prop('contentWindow');
			//清楚旧的iframe
			$('.mainContent').find('iframe.RuoYi_iframe').eq(0).attr('src', 'about:blank');
		    try{ 
		        iframe.document.write(''); 
		        iframe.document.clear(); 
		    }catch(e){} 
		    //把iframe从页面移除 
		    $('.mainContent').find('iframe.RuoYi_iframe').eq(0).remove(); 
